...
def compute_xp(df, max_gw=MAX_GW_HORIZON):
    ...
    xgi = out["position"].map(XGI_WEIGHT).fillna(0) * out["xgi_p90"]
...
selected_gw_label = st.sidebar.selectbox("🎯 Target GW", [f"GW{i}" for i in range(1, MAX_GW_HORIZON+1)],
...
menu_options = [
    "📊 Dashboard Overview", "🛡️ My Squad & Pitch View", "🔄 Transfer Planner",
    "💡 Transfer Recommendations", "📅 Multi-GW Transfer Plan", "📅 Fixture Analyzer",
    "🎯 Set-Piece Takers", "🔍 Player Explorer", "🎰 Chip Simulator"
]
...
if st.session_state.custom_squad_ids:
    st.sidebar.download_button("⬇️ Download squad", json.dumps({"ids": st.session_state.custom_squad_ids, "bank": st.session_state.bank_balance}), "plk_squad.json")
...
def select_xi(squad, xp_col="xP", formation=None):
    if squad.empty or len(squad) < 11: return squad, pd.DataFrame()
    ...
    if formation is None:
        ...
        for nd, nm, nf in LEGAL_FORMATIONS:
            if len(defs)<nd or len(mids)<nm or len(fwds)<nf or len(gkp)<1: continue
            xi = pd.concat([gkp.head(1), defs.head(nd), mids.head(nm), fwds.head(nf)])
            sc = xi[xp_col].sum()
            if sc > best: best, best_xi = sc, xi
        if best_xi is None: best_xi = squad.sort_values(xp_col, ascending=False).head(11)
    else:
        nd, nm, nf = formation
...
def generate_pitch(xi, bench, capt_id, vice_id=None, gw=1):
    ...
    def card(x, y, p, bench=False):
        fig.add_layout_image(dict(source=p.get("photo_url") or get_photo(p.get("photo","")),
            x=x-0.04, y=y+0.05, sizex=0.08, sizey=0.1, xref="x", yref="y", sizing="contain", layer="above"))
        is_c = p["id"]==capt_id
        is_v = vice_id and p["id"]==vice_id
        if is_c or is_v:
            fig.add_annotation(x=x+0.035, y=y+0.09, text="<b>C</b>" if is_c else "<b>V</b>",
                showarrow=False, font=dict(size=10,color="#000"), bgcolor="#FFD700" if is_c else "#00BFFF", borderpad=2)
        fig.add_annotation(x=x, y=y-0.04, text=f"<b>{p['web_name']}</b><br><span style='font-size:9px'>{get_next_fixture(p['team'],gw)}</span>",
            showarrow=False, font=dict(size=10,color="#111"), bgcolor="rgba(255,255,255,0.9)", borderpad=2)

    for pos, y in pos_y.items():
        ps = xi[xi["position"]==pos]
        xs = np.linspace(15,85,len(ps)) if len(ps)>1 else [50]
        for i,(_,p) in enumerate(ps.iterrows()): card(xs[i], y, p)
    if not bench.empty:
        xs = np.linspace(15,85,len(bench))
        for i,(_,p) in enumerate(bench.iterrows()): card(xs[i], -8, p, True)

    fig.update_layout(xaxis=dict(range=[-2,102],visible=False), yaxis=dict(range=[-18,135],visible=False,scaleanchor="x"),
        height=750, margin=dict(l=5,r=5,t=20,b=5), plot_bgcolor="#1a0a2e", paper_bgcolor="#1a0a2e")
    return fig
...
if menu == "📊 Dashboard Overview":
    ...
    st.dataframe(cap[["web_name","position","team_short","now_cost","selected_by_percent","xP","Capt xP","status_badge"]],
                 use_container_width=True, hide_index=True)

    st.subheader("🚀 Top 15")
    top = players_df.sort_values("xP", ascending=False).head(15)
    fig = px.bar(top, x="web_name", y="xP", color="position", text="xP", template="plotly_dark",
    ...
    if use_manual or not official:
        ...
        defs_ids = st.session_state.custom_squad_ids if st.session_state.custom_squad_ids and len(st.session_state.custom_squad_ids)==15 else (
            gkp.head(2)["id"].tolist()+deff.head(5)["id"].tolist()+mid.head(5)["id"].tolist()+fwd.head(3)["id"].tolist())
        c1,c2,c3,c4 = st.columns(4)
        with c1: sg = st.multiselect("GKP", gkp["id"], [i for i in defs_ids if i in gkp["id"].values][:2],
            format_func=lambda x: gkp.loc[gkp["id"]==x,"display_label"].values[0], max_selections=2)
        with c2: sd = st.multiselect("DEF", deff["id"], [i for i in defs_ids if i in deff["id"].values][:5],
            format_func=lambda x: deff.loc[deff["id"]==x,"display_label"].values[0], max_selections=5)
        with c3: sm = st.multiselect("MID", mid["id"], [i for i in defs_ids if i in mid["id"].values][:5],
            format_func=lambda x: mid.loc[mid["id"]==x,"display_label"].values[0], max_selections=5)
        with c4: sf = st.multiselect("FWD", fwd["id"], [i for i in defs_ids if i in fwd["id"].values][:3],
            format_func=lambda x: fwd.loc[fwd["id"]==x,"display_label"].values[0], max_selections=3)
        ids = sg+sd+sm+sf
        if len(ids)==15: st.session_state.custom_squad_ids = ids
    ...
    if len(full) >= 11:
        form_opts = {"Auto (best xP)":None,"3-4-3":(3,4,3),"3-5-2":(3,5,2),"4-3-3":(4,3,3),
        ...
        with ca: st_id = st.selectbox("Starter", xi["id"], format_func=lambda x: xi.loc[xi["id"]==x,"web_name"].values[0])
        with cb: b_id = st.selectbox("Bench", bench["id"] if not bench.empty else [], format_func=lambda x: bench.loc[bench["id"]==x,"web_name"].values[0] if not bench.empty else "")
        with cc:
            st.write(""); st.write("")
        ...
        if picks_data:
            for p in picks_data.get("picks",[]):
                if p.get("is_captain"): capt = p["element"]
        if capt not in xi["id"].values: capt = xi.sort_values("xP", ascending=False).iloc[0]["id"]
    ...
    for team, data in SET_PIECE_DATA.items():
        for role, takers in data.items():
            ...
            for name, pct in takers[:3]:
                ...
                rows.append({
                    "Team": team, "Role": role, "Player": p["web_name"] if p is not None else name,
    ...
    if st.button("📊 Rank teams: Easiest → Hardest", type="primary"):
        ...
        fig = px.bar(rdf, x="Team", y="Avg FDR", color="Avg FDR",
    ...
    ids = st.session_state.custom_squad_ids or ([p["element"] for p in picks_data.get("picks",[])] if picks_data else [])
    if len(ids) < 15:
    ...
    with c1: out_id = st.selectbox("Sell", active["id"], format_func=lambda x: active.loc[active["id"]==x,"display_label"].values[0])
    with c2:
        maxp = active.loc[active["id"]==out_id,"now_cost"].values[0] + bank
        tgts = players_df[(~players_df["id"].isin(ids)) & (players_df["now_cost"]<=maxp) & (players_df["position"]==active.loc[active["id"]==out_id,"position"].values[0])]
        in_id = st.selectbox("Buy", tgts["id"] if not tgts.empty else [], format_func=lambda x: tgts.loc[tgts["id"]==x,"display_label"].values[0] if not tgts.empty else "")
    with c3: pgw = st.selectbox("For GW", list(range(selected_gw_num, min(selected_gw_num+6,39))))
    if st.button("Add to plan") and in_id:
        po = players_df[players_df["id"]==out_id].iloc[0]
        pi = players_df[players_df["id"]==in_id].iloc[0]
        st.session_state.multi_gw_plan.append({"gw":pgw,"out":po["web_name"],"in":pi["web_name"],
            "cost":round(pi["now_cost"]-po["now_cost"],1), "xp":round(pi[f"GW{pgw}_xP"]-po[f"GW{pgw}_xP"],2)})
        st.rerun()
    if st.session_state.multi_gw_plan:
        ...
        if st.button("Clear plan"): st.session_state.multi_gw_plan = []; st.rerun()

elif menu == "💡 Transfer Recommendations":
    st.title("💡 Transfer Recommendations")
    ids = st.session_state.custom_squad_ids or ([p["element"] for p in picks_data.get("picks",[])] if picks_data else [])
    if len(ids)<15: st.warning("Need full squad"); st.stop()
    ...
    for _, o in squad.iterrows():
        tg = players_df[(players_df["position"]==o["position"]) & (~players_df["id"].isin(ids)) &
                        (players_df["now_cost"]<=o["now_cost"]+bank) & (players_df["status"].isin(["a","d"]))]
        if tg.empty: continue
        b = tg.sort_values("hx", ascending=False).iloc[0]
        gain = round(b["hx"]-o["hx"],2)
        if gain>0.25:
            recs.append({"out_id":o["id"],"in_id":b["id"],"Out":o["web_name"],"In":b["web_name"],
                         "Team":b["team_short"],"Cost":round(b["now_cost"]-o["now_cost"],1),"Gain":gain})
    if recs:
        for r in sorted(recs, key=lambda x:-x["Gain"])[:8]:
            ...
            if c[3].button("Apply", key=f"{r['out_id']}_{r['in_id']}"):
                ...
                st.rerun()
    else: st.info("No strong upgrades.")
    ...
    ids = st.session_state.custom_squad_ids or ([p["element"] for p in picks_data.get("picks",[])] if picks_data else [])
    if len(ids)<11: st.warning("Need squad"); st.stop()
    ...
    if diff: f = f[f["selected_by_percent"]<=own]
    if q: f = f[f["web_name"].str.lower().str.contains(q.lower(),na=False)]
    f = f.sort_values(selected_gw_col, ascending=False)
    gcols = [f"GW{i}_xP" for i in range(selected_gw_num, MAX_GW_HORIZON+1)]
    show = f[["web_name","position","team_short","now_cost","selected_by_percent","status_badge"]+gcols].head(40)
    show.columns = ["Player","Pos","Team","Price","Own%","Status"]+[f"GW{i}" for i in range(selected_gw_num, MAX_GW_HORIZON+1)]
